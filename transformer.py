...
class TransformerModel(nn.Module):
    def  __init__ (self, input_dim, output_dim, nhead, num_encoder_layers):
        ...
        self.embedding = nn.Linear(input_dim, embd_dim)
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=embd_dim, nhead=nhead), 
            num_layers=num_encoder_layers
        )
        self.fc = nn.Linear(embd_dim, output_dim)
...
writer = SummaryWriter(log_dir='./pth/transformer/tensorboard')
logging.basicConfig(filename='./pth/transformer/training.log', level=logging.INFO, format='%(asctime)s - %(message)s')
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 50 == 0:
        torch.save(model.state_dict(), output_pth+'transformer_weights_ep'+str(epoch+1)+'.pth')
        print('模型权重已保存到'+output_pth+'transformer_weights_ep'+str(epoch+1)+'.pth')

# 保存模型权重
torch.save(model.state_dict(), output_pth+'transformer_weights_final.pth')
print('模型权重已保存到'+output_pth+'transformer_weights_final.pth')

# 加载模型权重并推理
model = TransformerModel(input_dim=33, output_dim=2, nhead=4, num_encoder_layers=3).to(device)
model.load_state_dict(torch.load(output_pth+'transformer_weights_final.pth'))
model.eval()  # 设置为评估模式
